...
<div class="container max-w-3xl">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-[#5D605C]">Edit Berita</h1>
            <p class="text-sm text-gray-400 mt-0.5 font-mono">{{ $news->slug }}</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-2xl border border-[#E1E3DE] overflow-hidden">
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    @if($news->image_path)
                        <div class="mb-3 p-3 bg-gray-50 border border-[#E1E3DE] rounded-xl flex items-center gap-3">
                            <img src="{{ asset($news->image_path) }}"
                                 alt="Cover saat ini"
                                 class="w-20 h-14 object-cover rounded-lg border border-[#E1E3DE]">
                            <div>
                                <p class="text-xs font-semibold text-[#5D605C]">Foto Cover Saat Ini</p>
                    ...
                    <div id="dropZone"
                        ...
                        <div id="dropPlaceholder">
                            ...
                            <p class="text-sm text-gray-400">Klik atau seret gambar baru ke sini</p>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="12"
                              class="w-full border border-[#E1E3DE] rounded-xl px-4 py-3 text-sm text-[#5D605C] leading-relaxed focus:outline-none focus:ring-2 focus:ring-[#7B5556]/30 resize-y @error('content') border-red-400 @enderror">{{ old('content', $news->content) }}</textarea>
                    @error('content')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            {{-- Meta Info Bar --}}
            <div class="px-6 py-3 border-t border-[#E1E3DE] bg-gray-50/40 flex items-center gap-4 text-xs text-gray-400">
                <span>Dibuat oleh: <strong class="text-[#5D605C]">{{ $news->user->name ?? '—' }}</strong></span>
                <span>Pada: <strong class="text-[#5D605C]">{{ $news->created_at->format('d M Y, H:i') }}</strong></span>
            </div>
            ...
            <div class="px-6 py-4 border-t border-[#E1E3DE] bg-gray-50/60 flex justify-end gap-3">
                ...
                <button type="submit"
                        class="px-3 py-1.5 text-xs bg-pink-500 text-white rounded-lg hover:bg-pink-600 shadow-sm transition-colors font-medium">
                    <svg class="w-3.5 h-3.5 inline-block mr-1 -mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
