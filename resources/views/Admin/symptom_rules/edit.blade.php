...
@section('title', 'Edit Pertanyaan Anamnesis')
@section('content')
<div class="max-w-2xl mx-auto">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span class="text-[#7B5556] font-semibold">Edit #{{ $symptomRule->id }}</span>
    </nav>
    ...
    <div class="bg-white rounded-2xl border border-[#E1E3DE] overflow-hidden">
        ...
        <div class="flex items-center gap-3 px-6 py-5 border-b border-[#E1E3DE] bg-[#EBDBDD]/20">
            ...
            <div>
                <h2 class="font-bold text-[#5D605C] text-base">Edit Pertanyaan Anamnesis</h2>
                <p class="text-xs text-gray-400">
                    Kondisi:
                    <strong class="text-[#7B5556]">{{ $symptomRule->knowledgeBase?->nama_objek ?? '—' }}</strong>
                    &mdash; {{ $symptomRule->knowledgeBase?->tingkat_keparahan ?? '—' }}
                </p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.symptom-rules.update', $symptomRule->id) }}" method="POST" class="p-6 space-y-5">
            ...
            <div>
                ...
                <textarea id="pertanyaan" name="pertanyaan" rows="3"
                    ...
                    class="w-full px-3 py-2.5 border rounded-xl text-sm text-[#5D605C] resize-none focus:outline-none focus:ring-2 focus:ring-[#7B5556]/30
                        {{ $errors->has('pertanyaan') ? 'border-red-400' : 'border-[#E1E3DE]' }}">{{ old('pertanyaan', $symptomRule->pertanyaan) }}</textarea>
            ...
            {{-- Timestamps --}}
            <div class="text-xs text-gray-400 bg-[#F9F9F8] border border-[#E1E3DE] rounded-xl px-4 py-3 flex flex-wrap gap-4">
                <span>Dibuat: {{ $symptomRule->created_at->format('d M Y, H:i') }}</span>
                <span>Diperbarui: {{ $symptomRule->updated_at->format('d M Y, H:i') }}</span>
            </div>
