...
@section('title', 'Edit Pertanyaan Anamnesis — Admin')
...
<div class="max-w-2xl mx-auto">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <span class="text-gray-700 font-medium">Edit #{{ $symptomRule->id }}</span>
    </nav>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {{-- Header Card --}}
        <div class="flex items-center gap-3 px-6 py-5 border-b border-gray-100 bg-amber-50/60">
            <div class="w-10 h-10 rounded-xl bg-amber-100 flex items-center justify-center text-amber-500">
                <i class="fas fa-edit"></i>
            </div>
            <div>
                <h2 class="font-bold text-gray-800 text-lg">Edit Pertanyaan Anamnesis</h2>
                <p class="text-xs text-gray-500">
                    Kondisi: <strong>{{ $symptomRule->knowledgeBase?->nama_objek }}</strong>
                    — {{ $symptomRule->knowledgeBase?->tingkat_keparahan }}
                </p>
            </div>
        </div>

        {{-- Body Form --}}
        <form action="{{ route('admin.symptom-rules.update', $symptomRule->id) }}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="knowledge_base_id" name="knowledge_base_id"
                    ...
                    @foreach ($knowledgeBases as $kb)
                        ...
                        @endif
                        <option value="{{ $kb->id }}"
                            {{ (old('knowledge_base_id', $symptomRule->knowledge_base_id) == $kb->id) ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea id="pertanyaan" name="pertanyaan" rows="3"
                    ...
                    class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-400 text-sm resize-none
                        {{ $errors->has('pertanyaan') ? 'border-red-400' : 'border-gray-200' }}">{{ old('pertanyaan', $symptomRule->pertanyaan) }}</textarea>
            ...
            <div>
                ...
                <div class="flex items-center gap-4">
                    <input type="number" id="cf_gejala" name="cf_gejala"
                        value="{{ old('cf_gejala', number_format($symptomRule->cf_gejala, 1)) }}"
            ...
            {{-- Info timestamps --}}
            <div class="text-xs text-gray-400 bg-gray-50 rounded-xl px-4 py-3 flex gap-6">
                <span><i class="fas fa-clock mr-1"></i> Dibuat: {{ $symptomRule->created_at->format('d M Y, H:i') }}</span>
                <span><i class="fas fa-pencil-alt mr-1"></i> Diupdate: {{ $symptomRule->updated_at->format('d M Y, H:i') }}</span>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                ...
                <button type="submit"
                    class="px-6 py-2.5 rounded-xl bg-amber-500 text-white hover:bg-amber-600 text-sm font-semibold shadow transition">
